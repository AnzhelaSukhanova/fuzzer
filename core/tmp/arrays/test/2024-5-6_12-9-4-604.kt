...
// builderInferenceAndGenericArrayAcessCall.kt
// isKlib=false
// WITH_RUNTIME
...
interface Foo<T>{
inline  private  fun  rxtla(a: UByte, b: MapWithPlusOperator<Collection<Int?>, Array<Foo<UShort>>>, c: ULong): ULong = TODO()
}

class FooImpl<<T>> : Foo<T>

@OptIn(ExperimentalTypeInference::class)
fun <T> myflow(@BuilderInference block: FooT.() -> Unit): Foo<T> {
    val impl = FooImpl<T>()
    impl.block()
    return impl
}
...
class MapWithPlusOperator<K, V>(val m: MutableMap<K, V>):Foo<ULong> { 
}
...
fun box(): String {for (e in UIntRange(1.toUInt(), 89.toUInt())) { 
...
//Combined output:
//org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (14,16) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/builderInferenceAndGenericArrayAcessCall.kt
//
//Attachments:
//causeThrowable
...
//----
//expression.kt
//File name: builderInferenceAndGenericArrayAcessCall.kt Physical: true Injected: false
//@OptIn(ExperimentalTypeInference::class)
//fun <T> myflow(@BuilderInference block: FooT.() -> Unit): Foo<T> {
//    val impl = <caret>FooImpl<T>()
//    impl.block()
//    return impl
//}
