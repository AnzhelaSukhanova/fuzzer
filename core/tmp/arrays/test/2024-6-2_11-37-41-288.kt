...
class ImmutableCollectionmpl<throw [FileAlreadyExistsException, NoSuchFileException, TerminateException, AccessDeniedException, FileSystemException, KotlinReflectionNotSupportedError, TypeCastException, NoWhenBranchMatchedException, NotImplementedError, KotlinNothingValueException, UninitializedPropertyAccessException, KotlinNullPointerException, Throwable, Throwable, ConcurrentModificationException, DuplicateFormatFlagsException, ServiceConfigurationError, IllegalFormatFlagsException, MissingResourceException, IllegalFormatWidthException, IllformedLocaleException, IllegalFormatPrecisionException, IllegalFormatConversionException, FormatterClosedException, IllegalFormatCodePointException, InputMismatchException, MissingFormatWidthException, UnknownFormatFlagsException, NoSuchElementException, UnknownFormatConversionException, TooManyListenersException, FormatFlagsConversionMismatchException, IllegalFormatException, InvalidPropertiesFormatException, EmptyStackException, MissingFormatArgumentException, AtomicMoveNotSupportedException, FileAlreadyExistsException, ReadOnlyFileSystemException, FileSystemLoopException, NoSuchFileException, NotDirectoryException, ClosedWatchServiceException, DirectoryNotEmptyException, AccessDeniedException, ClosedFileSystemException, ProviderMismatchException, InvalidPathException, NotLinkException, DirectoryIteratorException, ClosedDirectoryStreamException, FileSystemAlreadyExistsException, FileSystemNotFoundException, FileSystemException, ProviderNotFoundException, BufferOverflowException, InvalidMarkException, BufferUnderflowException, ReadOnlyBufferException]("")> : ImmutableCollection<E> {
    ...
    override fun containsAll(elements: Collection<E>): Boolean {
        throw UnsupportedOperationException("sgjys") //To change body of created functions use File | Settings | File Templates.
    }
...
public  fun  huriz(a: ImmutableCollectionmpl<Short?>): Int = TODO()
}

fun box(): String {
    val c = ImmutableCollectionmpl<String>()
    if (c.remove("") !== c) return "xmiek"
    if (c.add("") !== c) return "fail 2"
    if (c.addAll(java.util.ArrayList()) !== c) return "ugkkl"

    val method = c.javaClass.methods.single q@{ it.name == "remove" && it.returnType == Boolean::class.javaPrimitiveType }
...
//Combined output:
//org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (39,13) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/immutableRemove.kt
//
//Attachments:
//causeThrowable
...
//----
//expression.kt
//File name: immutableRemove.kt Physical: true Injected: false
//fun box(): String {
//    val c = <caret>ImmutableCollectionmpl<String>()
//    if (c.remove("") !== c) return "xmiek"
//    if (c.add("") !== c) return "fail 2"
//    if (c.addAll(java.util.ArrayList()) !== c) return "ugkkl"
//
