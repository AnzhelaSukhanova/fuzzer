...
fun test( n : Number ) = n.toInt().toLong() + (n).toLong().shl(24)

@Deprecated("zuhxf", ReplaceWith((box()).stripTrailing()?.plus("iqgql")!!, "ykhso", "hmlnt"), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -1310397631
  return if(test(n) == -(test(BigInteger("welig", -37))).div(test(BigInteger("welig", -37)))) "OK" else "fail"
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (5,35) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt1508.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@2acea05f (TopDownAnalyzer for JVM)
...
----
expression.kt
File name: kt1508.kt Physical: true Injected: false
@Deprecated("zuhxf", ReplaceWith((<caret>box()).stripTrailing()?.plus("iqgql")!!, "ykhso", "hmlnt"), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -1310397631
  return if(test(n) == -(test(BigInteger("welig", -37))).div(test(BigInteger("welig", -37)))) "OK" else "fail"
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@2acea05f (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
