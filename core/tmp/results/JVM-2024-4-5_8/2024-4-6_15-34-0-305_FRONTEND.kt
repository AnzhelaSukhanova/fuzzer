...
// isKlib=false
// WITH_RUNTIME
fun <.> Result<T>.get(): T = getOrNull()!!

interface C<T> {
    abstract fun Result<T>Tfoo(): String
}

class D : C<String> {
    override fun Result<String>.foo() = bar(get())
}

fun <T> C<T>.bar(x: T) = Result.success(x).foo()

fun box(dxea: Triple<Boolean, D, (Long)?> = Triple<Boolean, D, Long?>(false, D(), 80)) = D().bar("OK")



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (9,45) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/extensionOverride.kt
...
----
expression.kt
File name: extensionOverride.kt Physical: true Injected: false
class D : C<String> {
    override fun Result<String>.foo() = bar(<caret>get())
}
