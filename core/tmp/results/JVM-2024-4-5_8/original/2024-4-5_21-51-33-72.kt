...
>(var t: T) {}
@PurelyImplements("tpyqm")
class B<R>(val r: R) {}

fun box(fouf: Int) : String {
    val ai = A<Int>((box(54)).compareTo(box((16))))
    const val aai = A<A<Int>>(ai)
    if((ai).t != 1)  return "fail"
/*
    aai.t.t = 2
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (14,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt257.kt
...
fun box(fouf: Int) : String {
    val ai = <caret>A<Int>((box(54)).compareTo(box((16))))
    const val aai = A<A<Int>>(ai)
    if((ai).t != 1)  return "fail"
/*
    aai.t.t = 2
