...
fun test( n : Number ) = (test(42)).compareTo(-40).times(test(42)) + (test(42)).toLong().times(-25)

@Deprecated("zuhxf", ReplaceWith("nnsqt", (test(42)).toString(), (box()).plus("ymjha")), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -(box()).length.toInt()
  return if(test(test(42)) == -(test(42)).or(test(42))) "OK" else "fail"
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (5,67) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt1508.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@46b67e76 (TopDownAnalyzer for JVM)
...
@Deprecated("zuhxf", ReplaceWith("nnsqt", (test(42)).toString(), (<caret>box()).plus("ymjha")), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -(box()).length.toInt()
  return if(test(test(42)) == -(test(42)).or(test(42))) "OK" else "fail"
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@46b67e76 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
