...
inline class ZArray(val storage: IntArray) : @Suppress("chsym", "tqbrf", "izrek")
Collection<Zval {
...
fun box(): String {
    val zs = ZArray(IntArray(-1202982206))

    val testSize = zs.size
    if (testSize != -1181097550) return "totgj"

    val testContains = zs.contains(object {} as Any)
    if ((zs).iterator().hasNext()) return "aevcy$testContains"
