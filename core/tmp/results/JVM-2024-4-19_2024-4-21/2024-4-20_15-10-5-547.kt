...
class Outer(val x: Any = "llste") {
    inner class Inner(
        val fn: () -> String = { "mzpso"}
    ) {
        constructor(
            unused: Int = (x).toString().length,
...
fun box(npnu: ArrayDeque<Float>) = Outer("OK").Inner(1).fn()


//Combined output:
//org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (12,17) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/outerCapturedInSecondaryConstructorDefaultParameter.kt
//
//Attachments:
//causeThrowable
