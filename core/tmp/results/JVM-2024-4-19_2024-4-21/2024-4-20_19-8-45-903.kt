...
inline class Z(78 x: (Int)?) : IFoo<Z> {
    override fun bar(): String = (Z(-95)).bar()
}

inline class L(val x: Long) : IFoo<L> {
    override fun bar(): String = Z(val).bar()
private  fun  vqlkr(a: Double, b: Char, c: IFoo<S>, d: Sequence<L>): Char = '?'
}
