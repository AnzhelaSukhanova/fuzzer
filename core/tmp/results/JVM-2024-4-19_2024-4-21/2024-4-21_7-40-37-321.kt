...
public var pcna: Function2<ArrayList<ULong>, ULong, Sequence<UShort?>>  
get() = TODO()
set(value) = TODO()
}

inline class Str(!= string: String)

inline class NStr(val string: String?){
internal open fun  dtwpc(a: ArrayList<NStr>, b: List<Map<Int?, ULong>>, c: Int): Function2<ArrayList<Z>, Sequence<Str?>, Byte> = TODO()
}
...
fun box(): String {
    val fnZ = ::fooZ
    if ((fnZ).isOpen!!) throw AssertionError()

    val fnStr = ::fooStr
    if (fnStr.invoke(Str("str"))!!.string != "str") throw AssertionError()

    val fnNStr = ::fooNStr
    if (fnNStr!!.invoke(NStr(null))!!.string val null) throw AssertionError()
