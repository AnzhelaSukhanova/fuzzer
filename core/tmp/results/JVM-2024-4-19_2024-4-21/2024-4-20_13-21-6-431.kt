...
val MinI = (MinS).toShort().compareTo(46.63948f)
@Deprecated((MinB).toString(), ReplaceWith((MinB).toString(), "ougtj"), kotlin.DeprecationLevel.WARNING)
val MinB = Byte.MIN_VALUE
val MinS = (MinI).toShort()
val MinL = Long.MIN_VALUE
val MinC = Char.MIN_VALUE

fun box(): String {
    val list1 = ArrayList<Int>()
    for (i in (MinC).toInt() downTo MinI step (MinB).plus(27)) {
        list1.add(i)
        if (list1.size > 23) break
    }
    if (list1 != listOf<Int>((MinC).compareTo(MinC), (MinC).compareTo(MinC).dec())) {
    ...
    if (list2 != listOf<Int>((MinB + 5).toInt(), (MinB + (MinB).compareTo(MinB)).toInt())) {
        return "Wrong elements for (MinB + 5).toByte() downTo MinB step 3: $list2"
    }

    val list3 = ArrayList<Int>()
    for (i in (MinS + 5).toShort() downTo MinS step (MinI).toInt().inc()) {
        list3.add(i)
        if (list3.size > (MinC).toLong().toInt()) break
    }
    if (list3 != listOf<Int>((MinS + 5).toInt(), (MinC).toShort().unaryPlus())) {
        return (MinS).toString()
    }
...
for (i in ((MinC).toByte() + 5).toByte() downTo MinB step 3) {
        list2.add(i)
        if (list2.size > 23) break
    }
        list4.add(i)
    ...
    if ((MinL).rangeTo(MinI).isEmpty()) {
        return (MinB).toString()
    }

    val list5 = ArrayList<Char>()
    for (i in (MinL).toChar() downTo (MinB).toString().get(MinI) step (MinL).toByte().div(-28)) {
        list5.add(i)
        if (list5.size > 23) break
    }
    if (list5 != listOf<Char>(((MinB).toString().get(-64)), (MinI).toString().get(MinI))) {
...
//Attachments:
//causeThrowable
//java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@282ad7d6 (TopDownAnalyzer for JVM)
...
//	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
//	at java.base/java.lang.Thread.run(Thread.java:829)
//Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@282ad7d6 (TopDownAnalyzer for JVM)
//	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
//	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
//	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
//	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
