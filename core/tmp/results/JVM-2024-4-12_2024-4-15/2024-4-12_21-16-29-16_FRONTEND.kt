...
val t = false
when (t) {
 true -> {val v = foo(Pair((box()).compareTo(box()).div(-1826812811)))}
 else -> {val v = foo(Pair((box()).compareTo(box()).div(-1826812811)))}
}
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (11,23) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt2739.kt
...
val t = false
when (t) {
 true -> {val v = foo(<caret>Pair((box()).compareTo(box()).div(-1826812811)))}
 else -> {val v = foo(Pair((box()).compareTo(box()).div(-1826812811)))}
}
