...
internal final class Txh  (var dzr: Int, vararg ntx: BaseGeneric<UShort>, val umu: HashSet<ULong> = LinkedHashSet<ULong>(71, 26.218285f), smo: Function2<List<Float>, Long, Derived<UShort>>, kfu: Derived<Short>){


inline suspend private  fun  dgujf(a: Derived<Derived<BaseGeneric<Float>>>, b: HashMap<Function2<ArrayList<List<Char>>?, Set<BaseGeneric<HashMap<BaseGeneric<Derived<Float>>?, Short>>>, BaseGeneric<Derived<Derived<Long>>>>, BaseGeneric<Float>>, c: Sequence<String>): List<Boolean> = TODO()

inline tailrec private  fun  vmpdl(a: BaseGeneric<Set<Derived<String>>?>, b: Derived<String>, c: Pair<Boolean, HashMap<Char, Char>>, d: BaseGeneric<BaseGeneric<String>>): Boolean = TODO()

protected  fun  yoqnz(a: MutableMap<MutableMap<BaseGeneric<UByte>, ULong?>, BaseGeneric<Boolean>>): UByte = TODO()
...
class Derived<T>(t: List<T>) : BaseGeneric<@UnsafeVariance()
List<T>>(t) {
    var test = 0

    override fun iterate() {
        test = 0
        b@for (i in t.indices) {

test = 0
            test = test * 10 + (i + 1)
}
    }
internal  fun <T>  lrkhk(a: Derived<LinkedHashSet<LinkedHashSet<Short>>>?): Pair<Long, Boolean> = TODO()
}
