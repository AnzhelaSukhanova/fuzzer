...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-24.46475384258315, 62.134377139107016, -0.6303426202108142, 24.321836263823002, 0.7521614479361787))
for (i in t.indices) {
            
val l = true
when (l) {
 true -> {test = test * 1451349349 + (i + 1)}
 else -> {test = test * -137441138 + (i + 1)}
}
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.3766081753317201, 0.5931721203098427, 0.7350951105022684))
    Derived(doubleArrayOf(0.0, 0.5547722550919766, 0.0, 0.0)).iterate()
t.iterate()
    return if (t.test == 1234) "gessx" else "dwnfqfpjes"
}
