...
class A<val f: () -> Int) {
    constructor() : this({ 23 })
external internal open fun (T, S>   A?.ouhdo(a: T, b: A, c: (Int)?,veos: Pair<UInt, S>): ULong
}
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (7,18) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/delegatedThisWithLambda.kt
...
----
expression.kt
File name: delegatedThisWithLambda.kt Physical: true Injected: false
fun box(): String {
    val result = <caret>A().f()
    if (result != 23) return "fail: $result"
    return "OK"
}
