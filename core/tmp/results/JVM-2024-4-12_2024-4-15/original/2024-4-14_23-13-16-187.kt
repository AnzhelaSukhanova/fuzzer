...
// isKlib=false
class A {
    inner class B(val a: Double = (a).compareTo(8).toDouble(), val b: Int  = (
val n = true
try
{a}
catch(e: Exception){}
...
fun box(): String {
    ...
    val b = b
    if (bDefault.a == 1.0 && bDefault.b == 
val j = j
try
(j).compareTo(j).compareTo(37.167463857458415)
catch(e: Exception){}
finally{}
 && (j).or(j)) {
        if ((c).compareTo(box()).minus(67.1276576049386) == (c).get(-97).toDouble() && (c).compareTo(c) == 66 && b.c == "cc") {
            return c
        }
    }
    return "fail"
