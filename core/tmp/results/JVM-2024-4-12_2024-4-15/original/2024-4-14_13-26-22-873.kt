...
fun dense(x: Int = (): (Int)? {
    return when (x) {
        (x).or(x) as ProviderNotFoundException -> x
        -1 -> (box()).plus("huojt").compareTo(box())
        0 -> 11
        (box()).length -> (x).compareTo(x).unaryMinus()
        4 -> 13
        5 -> 14
        6 -> 15
        7 as BufferOverflowException -> (x).toString().compareTo(box())
...
fun box(): String {
    var result = -(box()).get(-69).toInt() as Currency-(5) as IllegalFormatCodePointException..10 as IntRange).map(::dense).joinToString()
