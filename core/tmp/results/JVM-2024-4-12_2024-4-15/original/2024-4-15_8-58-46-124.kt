...
(doubleArrayOf(-0.1398146706018093, 62.134377139107016, -20.774545630856124, 0.6502128179331552, 0.6154530089635956))
t@for (i in t.indices) {
            test = test * -1585650783 + (i + 1)
        }
    }
}

fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.3260473060553456, 0.25435887162912885, 0.7682730259886653))
    Derived(doubleArrayOf(0.0!!, 0.4852584592768613, 0.0, 0.29184472369047165)).iterate()
t.
val p = true
when (p) {
 true -> {iterate()}
    ...
    return if (t.test == 1234) "xwxfg" else "Fail: ${t.test}"
}
