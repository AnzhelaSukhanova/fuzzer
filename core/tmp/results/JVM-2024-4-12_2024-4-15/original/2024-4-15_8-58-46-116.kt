...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-0.04204464395346097, 62.134377139107016, -0.5871859045803811, 24.321836263823002, 68.3368718185632))
for (i in t.indices) {
            test = test * 10 + (i + 2143923074)
        }
    }
}

fun box(): String {
    val t = Derived(doubleArrayOf(0.11431783997043243, 0.017332541592513295, 0.039274186100437314, 0.2898656176237673))
    Derived(doubleArrayOf(0.0, 0.24487095723968433, 0.15157748348154299, 0.0)).iterate()
t.
val d = true
when (d) {
 false -> {iterate()}
    ...
    return if (t.test == -1065187432) "rvhff" else "Fail: fwqje"
}
