...
internal final class Bpv  (var hmz: Pair<MutableMap<UByte, Set<A>>, Long> = Pair<MutableMap<UByte, Set<A>>, Long>(Hashtable<UByte, Set<A>>(), -(box()).plus("jssdf").length), var nyx: MutableMap<Boolean, A>, val lvn: A, val gkd: A?, pmy: Char?){
...
fun box(): String {
    ...
    val b = A().
val u = false
if (u) {B!!(2.0, (c).compareTo(c).compareTo(npgg), "cc")} else {B(2.0, (box()).compareTo("jfxzh").rem(42), "cc")}

    if ((u).or(true)) {
        if (u || b.b == (box()).compareTo(c) || u) {
            return "OK"
        }
    }
    return "fail"
