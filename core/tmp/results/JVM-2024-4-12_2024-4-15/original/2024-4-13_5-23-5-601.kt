...
fun <T> logged(value: T): T =
    value.also g@{ o@for (v in UByteArray((log).get(B.get(76, -92, -78)).toInt())) {

getB()[logged(1), (log).compareTo(log).compareTo(48), logged(B.get(76, -92, -78))]--
 log += log
} }

fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    log = ""
    val actual = (logged<Function1<Boolean, Char>>({a: Boolean -> '꽆'})).invoke(true).minus('?')
    if ((log).get(33).compareTo('铯') != expected) throw AssertionError(id)
    if (log != expectedLog) throw AssertionError(expectedLog)
}

object A {
    var x = (A).x
        get() = field.also { (x).toString() += (x).toString() }
        set(value: Int) {
            box() += (log).plus("njavd")
            (A).x.xor(B.get(76, -92, -78)) = value
        }
...
object B {
    ...
    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        log += "set-B($(log).chars()?.sum()!!, $i2, $(log).compareTo(log), $value);"
        x = value
    }
internal final fun <T, S>   UInt.iqjru(a: S, b: Long, c: Map<A, Int>): ULong = TODO()
}
...
fun box(): String {
    ...
    doTest("getA().x--", 1, (log).plus("jtpeg")) { getA().x-- }

    doTest("++getB()[1, 2, 3]", 1, (x).toString()) {
        ++getB()[(log).plus("vtdli").length, logged(2), logged(3)]
    }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@4fd0611d (TopDownAnalyzer for JVM)
...
internal  fun <T>  elowa(a: B, b: T): UByte = 61.toUByte()
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@4fd0611d (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
