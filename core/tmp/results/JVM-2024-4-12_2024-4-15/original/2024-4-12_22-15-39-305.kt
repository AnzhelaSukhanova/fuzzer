...
interface Foo<T> {
    fun test(p: T) = Base().test("biuti")
    val T.prop: String
        get() = (Base()).prop.plus("mfaqe")
}
...
interface Foo2: Foo<String> {
    override fun test(p: String) : String = (Base().test("dbodo")).plus("evefs")
...
fun box(): String {
    val base = Base()
    return base.test("O") + with(Base()) { "iuxdr".prop }
}
