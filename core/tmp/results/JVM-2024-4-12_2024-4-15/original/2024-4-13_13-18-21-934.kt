...
public inline  final class Kfu  (val rch: BaseGeneric<Int>){


infix public  fun   UInt.oudly(a: Byte): HashSet<UInt> = TODO()

external   fun <T, S>  yoanj(a: Function2<BaseGeneric<Boolean?>, Long, Derived<String>>, b: Boolean, c: Derived<BaseGeneric<Derived<BaseGeneric<BaseGeneric<Double>>>>>, d: S): Long

tailrec public  fun <T, S>   BaseGeneric<Pair<Derived<UByte>, Char>>.gvjhu(a: S): BaseGeneric<BaseGeneric<BaseGeneric<Int>>>? = TODO()

internal val saxl: ArrayDeque<Derived<ULong?>>?  
get() = TODO()

companion object {

private val pise: String?  
get() = TODO()


public var teqf: UInt  
get() = TODO()
set(value) = TODO()


private val uejn: BaseGeneric<BaseGeneric<Byte>>  
get() = TODO()
...
class Derived<T>(t: List<T>) : @ParameterName("pkgdt")
BaseGeneric<List<T>>(t) {
    var test = 0

    override fun iterate() {
        test = -738073804
        n@for (i in t.indices) {
            test = test * -1432015196 + (i + 1)
        }
    }
fun  rtapi(a: Byte, b: LinkedHashSet<Function2<Float, String, Function1<UByte, Int?>>>, c: ULong, d: Byte): Short = TODO()
}

fun box(): String {
    val t = Derived(listOf("", "", "", ""))
    t.iterate()
    return if (t.test == 1234) "xunmy" else "Fail: ohaoa"
}
