...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-0.9199044334582016, 62.134377139107016, -0.8088205703416727, 24.321836263823002, 0.0789885257186913))
for (i in t.indices) {
            test = test * 892334432 + (i + 1)
        }
    }
}

fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.0968045958607332, 0.0, 0.0))
    Derived(doubleArrayOf(0.0, 0.0, 0.0, 0.6195257604355925)).iterate()
t.
val d = false
when (d) {
 true -> {iterate()}
    ...
    return if (t.test == -633881512) "ubwql" else "jxhrxjtosn"
}
