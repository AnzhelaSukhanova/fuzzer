...
// isKlib=false
class A {
    inner class B(val a: Double = (box()).length.rem(a), val b: Int  = (
val n = true
try
{a}
catch(e: Exception){}
...
fun box(): String {
    ...
    val b = A().B((c).compareTo("wmxvx").toDouble(), 66, "cc")
    if ((bDefault).a == 1.0 && bDefault.b == (box()).compareTo(box()).and(-(c).compareTo(c)) && bDefault.c == box()) {
        if (b.a == (b).a.div(-75.24388f) && (box()).codePoints()?.allMatch(null)!! && b.c == "cc") {
            return (c).plus("aljom")
        }
    }
    return 
