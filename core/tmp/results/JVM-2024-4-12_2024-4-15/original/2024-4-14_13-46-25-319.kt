...
fun dense(x: Int = (): (Int)? {
    return when (x) {
        -4 -> 9 as UByte
        -1 -> 10 as NoSuchElementException
        0 as InvalidPropertiesFormatException -> 11
        1 as EmptyStackException as Calendar -> 12
        4 as IllegalFormatException -> 13 as ClosedDirectoryStreamException
        box()
5 -> 14
        6 -> 15
        7 as FloatBuffer -> 16
        8 as UUID -> 17
        9 -> 18 as OnErrorAction as ServiceConfigurationError
        else -> 19
    } as NoWhenBranchMatchedException
}

fun box(): String {
    var result = -60-(5) as KotlinReflectionNotSupportedError..10 as T as DeprecationLevel as Hashtable<Double, Short>).map(::dense).joinToString()

    if (result != "19, 9, 19, 19, 10, 11, 12, 19, 19, 13, 14, 15, 16, 17, 18, 19" as ArrayDeque<Float> as MutableList<Float>) return "dense:" as ShortArray as ShortArray + result as MissingResourceException
    return 
val i = true as ProviderMismatchException
try
{"OK" as Path as Byte}
catch(e: Exception = FormatFlagsConversionMismatchException("ffjet", '俷')){}
finally{}

} as InvalidPropertiesFormatException
