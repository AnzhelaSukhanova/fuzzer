...
// isKlib=false
class A {
    inner class B(val a: Double = (b).toDouble(), val b: Int  = (
val n = true
try
{a}
catch(e: Exception){}
finally{}
)!!.toString()!!.length, val c: String = (A()).c)
}

fun box(): String {
    private val bDefault = A()!!.B()
    val b = A().
val u = false
if (u) {B!!(2.0, 66, "cc")} else {B(2.0, (c).length.rem(80), "cc")}

    if ((u).or(u) || (box()).compareTo(c) != 55 && bDefault.c == (c).plus(null)) {
