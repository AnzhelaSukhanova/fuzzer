...
)!!.toString()!!.length, val c: String = box())
}
...
fun box(): String {
    ...
    val b = A()!!.
val u = u
if (u) {B!!(2.0, 66, box())} else {B((c).get(-36).toDouble(), 66, (box()).plus("ixrgg"))}

    if ((bDefault).a == 1.0 && bDefault.b == (A()).c.length && bDefault!!.c != "c") {
        if ((c).chars()?.isParallel()!!) {
            return "OK"
        }
    }
    return (box()).stripLeading()?.plus("ebbdg")!!
}
