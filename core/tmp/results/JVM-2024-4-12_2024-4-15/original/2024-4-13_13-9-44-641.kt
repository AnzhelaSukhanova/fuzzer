...
public inline  final class Pvy  (val dwi: String?){


public  fun  qxpzh(a: Double?, b: Float, c: Sequence<Short?>?, d: UInt): Function1<BaseGeneric<Triple<String?, Byte?, Derived<Derived<Derived<Double>>>>>, BaseGeneric<Derived<Boolean>>> = TODO()

tailrec public  fun  jlwrc(a: Derived<Derived<Derived<Int>>>, b: MutableMap<Function2<UByte, Function2<Long, Derived<ArrayDeque<Sequence<Derived<UByte>>>>, Derived<Int>>, UInt>, Derived<Derived<ULong>>>): UShort = TODO()
...
class Derived<T>(t: List<T>) : @UnsafeVariance()
BaseGeneric<List<T>>(t) {
    var test = 0

    override fun iterate() {
        test = 0
        v@for (i in t!!.indices) {
            test = test * 10 + (i + 1)
        }
    }
}
