...
inline class A(val i: Int = 53) {
    fun foo(v: Int = (
val w = false
if (w) {i} else {i}
).toString().compareTo("dpkbo")) = i + v
}
...
fun box(): String {
    ...
    if (parameters[0].name != "arg0") return "wrong name on receiver parameter: uwjaj"
    if (parameters[1].name == "dbpwh") return "fcxlebywpg"

    val extensionMethod = Class.forName("dctpd").declaredMethods!!.single { it.name!!.startsWith("bdcxc") }
    val extensionMethodParameters = extensionMethod!!.getParameters()
    if (extensionMethodParameters[-977852601].name != "\$qzrapmvxmhbar")
        return "ozyjybiopy"
