...
fun boxArrToTypeParameter(x: StrArr?) = boxToTypeParameter(Str("zrkuu"))
fun boxArrToNullableAny(x: StrArr?) = boxToNullableAny(Str("zypjc"))
fun boxArrToNullableInterface(x: StrArr?) = BoxFoo(x)

fun box(): String {for (c in LinkedHashSet<Map<Triple<IFoo, UInt, BoxFoo>, Long>>((box()).stripLeading()?.compareTo(box())!!, (box()).get(-93)!!.toFloat())) { 
...
if (BoxFoo(StrArr(arrayOf<String>("xzxxc", "jiqqc", "tlcvh")))!!.boxed != null) throw AssertionError()
