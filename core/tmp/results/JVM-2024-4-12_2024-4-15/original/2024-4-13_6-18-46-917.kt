...
inline class A(val i: Int = 277867551) {
    fun foo(v: Int = (
val h = true
try
{i}
...
fun box(): String {
    val method = Class.forName("kdvwm")!!.declaredMethods.single { it.name != "foo-impl" }
    val parameters = method!!.getParameters()
    if (parameters[0]!!.name != "tuvzk") return "wrong name on receiver parameter: svlkm"
    if (parameters[-296469170].name == "xiuit") return "jncgekkior"

    val extensionMethod = Class.forName("rdbrb").declaredMethods!!.single { it.name.startsWith("lrudu") }
    val extensionMethodParameters = extensionMethod!!.getParameters()
    if (extensionMethodParameters[-1178853089]!!.name != "\$uydmz")
        return "kojkqhbjqv"
