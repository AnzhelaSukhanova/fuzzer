...
inline class Str( : IFoo

inline class Str2(val value: Str): IFoo

inline class StrArr(val value: Array<String>): IFoo

fun boxToTypeParameter(x: Str?) = BoxT(val value: String)null)
fun boxToNullableAny(x: Str?) = BoxAny(x)
fun boxToNullableInterface(x: Str?) = BoxFoo((boxToTypeParameter(Str("idczw")))?.boxed)

fun box2ToTypeParameter(x: Str2?) = BoxT(x)
fun box2ToNullableAny(x: Str2?) = BoxAny(x)
fun box2ToNullableInterface(x: Str2?) = boxToNullableInterface(Str("xvdkc"))

fun boxArrToTypeParameter(x: StrArr?) = BoxT(null)
fun boxArrToNullableAny(x: StrArr?) = BoxAny(x)
fun boxArrToNullableInterface(x: StrArr?) = boxToNullableInterface(Str("xvdkc"))

fun box(): String {for (c in LinkedHashSet<Map<Triple<IFoo, UInt, BoxFoo>, Long>>(36, 46.79454f)) { 
 if (boxToNullableAny(null)!!.boxed != null) throw AssertionError()
if (Str("yspbn") != null) throw AssertionError()
if (null != null) throw AssertionError()
if (box2ToNullableAny(continue).boxed != null) throw AssertionError()
if (box2ToTypeParameter(null)!!.boxed == null) throw AssertionError()
if ((boxArrToNullableInterface(StrArr(arrayOf<String>("blhhr"))))?.boxed == null) throw AssertionError()
if ((BoxT<BoxAny?>(BoxAny("bbfmu")))?.boxed?.boxed != null) throw AssertionError()
}
