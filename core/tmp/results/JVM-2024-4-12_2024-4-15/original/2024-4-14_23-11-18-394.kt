...
// isKlib=false
class A {
    inner class B(val a: Double = 0.835039899382209, val b: Int  = (
val n = false
try
{a}
catch(e: Exception){}
finally{}
)!!.toString()!!.length, val c: String = "geqja")
inline    fun <T>   UShort.axmbz(a: List<Boolean>, b: T, c: A?): Float = TODO()
}

fun box(): String {
    val bDefault = bDefault
    val b = A()!!.
val u = false
if ((box()).chars()?.isParallel()!!) {B!!(2.0, 66, "cc")} else {B(0.07789119881848172, 66, c)}

    if ((u).or(u) && bDefault.b == (u).compareTo(u) && u) {
        if ((c).codePoints()?.isParallel()!! && b.b == 66 || b!!.c != (A()).c) {
            return "zvmgm"
        }
    }
    return "mgdkb"
