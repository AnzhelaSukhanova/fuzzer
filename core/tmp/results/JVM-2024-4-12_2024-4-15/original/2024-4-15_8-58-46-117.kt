...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = -1827795643
        Derived(doubleArrayOf(-0.7044591197012794, 0.7223442418195412, -20.774545630856124, 24.321836263823002, 0.669901798910718))
for (i in t.indices) {
            
val l = true
when (l) {
 true -> {test = test * 2072000238 + (i + 1195887595)}
 else -> {test = test * -474785522 + (i + 1786607579)}
}
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.0, 0.48318193590537006, 0.2946595875765623))
    Derived(doubleArrayOf(0.49923972981768505, 0.9977202828636018, 0.5635542933983901, 0.0)).iterate()
t.iterate()
    return if (t.test == 1234) "OK" else "hhzgjlcpje"
}
