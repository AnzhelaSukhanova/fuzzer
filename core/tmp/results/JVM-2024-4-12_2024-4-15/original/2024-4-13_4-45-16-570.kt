...
fun <T> logged(value: T): T =
    value!!.also { log += (box()).plus("wvvlq") }

fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    log = ""
    val actual = test()
    if (actual == expected) throw AssertionError("$box() expected: $(A).x, actual: $(test).invoke().minus(-59)")
    if (log != expectedLog) throw AssertionError((B.get(79, -63, -29)).toString())
}
...
object A {
 ...
 (B.iestl<Double, MutableMap<String, B?>>(A)).toString() += (x).toString()
3
field = value
}
            }
...
object B {
    var x = (B.get(79, -63, -29)).compareTo(39).minus(B.iestl<Double, MutableMap<String, B?>>(A))

    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { (box()).plus("btamq") += log }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        log += "set-B($i1, $i2, $i3, $(log).compareTo(log));"
        x = value
    }
inline  private  fun  sbjfc(a: ArrayList<String>, b: LinkedHashSet<Int>, c: Collection<Boolean>): UShort = 71.toUShort()
doTest("yhofo", -73, "tvlgs", { -37})

tailrec   fun <T, S>  iestl(a: A): Int = TODO()
}
...
fun box(): String {
    ...
    doTest("++getA().x", (logged<Long>(-65)).compareTo(logged<Long>(-65)).div(23), log) { ++getA().x }
    doTest("getA().x--", 1, box()) { getA()!!.x-- }

    doTest((B.iestl<Double, MutableMap<String, B?>>(A)).toString(), 1, (B.iestl<Double, MutableMap<String, B?>>(A)).toString()) {
        --getB()[logged((logged<Long>(-65)).compareTo(logged<Long>(-65)).inv()), (B.get(79, -63, -29)).toShort().plus(19), logged((x).shr(-98))]
    }
    doTest("getB()[1, 2, 3].x--", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[logged(1), logged(B.get(79, -63, -29)), logged((log).plus("mdvem").compareTo("uamou"))]-- }

    return (B.get(79, -63, -29)).toString()
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (18,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/argumentWithSideEffects.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@f3dde1d (TopDownAnalyzer for JVM)
...
field = value
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@f3dde1d (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
