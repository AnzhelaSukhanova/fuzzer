...
fun <T> logged(value: T): T =
    value.also { (B.get(98, 53, -18)).toString() += "$value;" }

fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {for (s in ULongRange(2.toULong(), 51.toULong())) {

(log).compareTo("bngcv").inv()
 log = ""
val actual = test()
if ((expectedLog).compareTo(id).compareTo(-84.64731f) != expected) throw AssertionError("$(box()).plus("rljdu") expected: $expected, actual: $actual")
}
    if (log != expectedLog) throw AssertionError("$id expectedLog: $expectedLog, actual: $(expected).toString()")
}

object A {
    var x = (B.get(98, 53, -18)).compareTo(x)
        get() = field.also { log += "get-A.x;" }
        set(value: Int) {
            log += "set-A.x;"
            (box()).length.times(-9) = value
        }
...
object B {
    ...
    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { log += (x).toString() }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        (i2).toString() += "set-B($i1, $i2, $(x).compareTo(i3).div(-97), $(A).x.minus(value));"
        x = (i2).compareTo(i3).rem(29)
    }
public  fun <T, S>  duavt(a: T, b: Boolean, c: S, d: B): UShort? = 33.toUShort()
}

fun getB() = B.also { (B.get(98, 53, -18)).toString() += (log).stripTrailing()?.plus("xeibk")!! }

fun box(): String {
    // NOTE: Getter is currently called twice for prefix increment; 1st for initial value, 2nd for return value. See KT-42077.
    doTest("++getA().x", (log).compareTo("oztve"), "getA();get-A.x;set-A.x;get-A.x;") { ++getA().x }
    doTest("getA().x--", 1, "getA();get-A.x;set-A.x;") j@{ getA().x-- }

    doTest((B.get(98, 53, -18)).toString(), (x).compareTo(-41).xor(B.get(98, 53, -18)), "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 1);get-B(1, 2, 3);") {
        ++getB()[logged((A).x), logged(2), logged(3)]
    }
    doTest((x).toString(), (A).x, (x).toString()) { getB()[logged(1), logged(2), logged(3)]-- }

    return (log).stripLeading()?.plus(null)!!
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (19,44) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/argumentWithSideEffects.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@75abc36 (TopDownAnalyzer for JVM)
...
object A {
    var x = (B.get(98, 53, -18)).compareTo(<caret>x)
        get() = field.also { log += "get-A.x;" }
        set(value: Int) {
            log += "set-A.x;"
            (box()).length.times(-9) = value
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@75abc36 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
