...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-24.46475384258315, 62.134377139107016, -0.929993696733123, 0.04566572890408627, 0.8107598386287489))
for (i in t.indices) {
            
val l = false
when (l) {
 false -> {test = test * 10 + (i + 553766409)}
 else -> {test = test * 10 + (i + 511899881)}
}
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.7837168545306366, 0.0, 0.0, 0.0))
    Derived(doubleArrayOf(0.09452823324561055, 0.0, 0.1664100781122364, 0.17866146100858094)).iterate()
t.iterate()
    return if (t.test == 239176857) "qlwnp" else "abeqt${t.test}"
}
