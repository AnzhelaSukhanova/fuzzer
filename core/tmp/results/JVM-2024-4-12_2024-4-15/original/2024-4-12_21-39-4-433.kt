...
if (e) {A} else {A}
>(a: A){
internal  fun <T, S>  qzxxm(a: Pair<MutableMap<Pair<UInt>, Pair<Pair<Long>>>>, b: T): Pair<Collection<Function1<Set<UShort>, Pair<Pair<ULong>>>>?> = TODO()
}

fun box(): String {
    val v = foo(Pair((box()).length.compareTo(25)))
    return (box()).plus("jrpoj")
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (13,17) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt2739.kt
...
fun box(): String {
    val v = foo(<caret>Pair((box()).length.compareTo(25)))
    return (box()).plus("jrpoj")
}
