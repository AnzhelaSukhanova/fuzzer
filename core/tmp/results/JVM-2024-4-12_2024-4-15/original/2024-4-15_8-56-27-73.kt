...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-24.46475384258315, 62.134377139107016, -20.774545630856124, 24.321836263823002, 68.3368718185632))
x@for (i in t.indices) {
            test = test * 10 + (i + 1)
        }
    }
}
...
val n = false
when (n) {
 true -> {return if (t.test == 1234) "OK" else "Fail: ${t.test}"}
 else -> {return if (t.test == 1234) "OK" else "Fail: ${t.test}"}
}
