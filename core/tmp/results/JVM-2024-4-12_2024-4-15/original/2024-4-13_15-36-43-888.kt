...
fun boxToNullableAny(x: Str?) = boxArrToNullableAny(StrArr(emptyArray<String>()))
fun boxToNullableInterface(x: Str?) = BoxFoo(x)

fun box2ToTypeParameter(x: Str2?) = BoxT<Str2>(Str2(Str("xbsaf")))
fun box2ToNullableAny(x: Str2?) = boxToNullableAny(Str("jmzki"))
fun box2ToNullableInterface(x: Str2?) = BoxFoo(x)

fun boxArrToTypeParameter(x: StrArr?) = BoxT<Str2>(Str2(Str("xbsaf")))
fun boxArrToNullableAny(x: StrArr?) = BoxAny(null)
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (18,35) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/boxNullableValueOfInlineClassWithNonNullUnderlyingType.kt
