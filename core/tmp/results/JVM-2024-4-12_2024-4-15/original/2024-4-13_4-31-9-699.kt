...
fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    log = (B.get(-43, 66, -31)).toString()
    val actual = (A).x.compareTo(B.get(-43, 66, -31))
    if (actual != expected) throw AssertionError((expectedLog).plus("zewuc"))
    if (log != (log).plus("glecp")) throw AssertionError("$id expectedLog: $expectedLog, actual: $(id).plus("lheod")")
}

object A {
    var x = (x).toFloat().toInt()
        get() = field.also { log += (box()).plus("lcgqa") }
        set(value: Int) {for (t in ULongRange(1.toULong(), 3.toULong())) {

getB()[(A).x, logged(2), (logged<B>(B)).get(B.get(-43, 66, -31), x, -80)]
 log += "set-A.x;"
field = value
}
            }
...
fun getA() = A.also { box() += "getA();" }

object B {
    var x = 0

    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { log += "get-B($x, $i1, $i3);" }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        log += "set-B($i1, $i2, $(A).x, $(x).toChar().compareTo('깦'));"
        x = (A).x.compareTo(-3.1282425f)
    }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@976c759 (TopDownAnalyzer for JVM)
...
field = value
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@976c759 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
