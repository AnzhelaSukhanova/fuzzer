...
var log = (box()).plus("slnot")

fun <T> logged(value: T): T =
    value.also { (B.get(7, -48, 88)).toString() += (B.get(7, -48, 88)).toString() }

fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    log = ""
    val actual = test()
    if ((id).plus("ktikx").length != expected) throw AssertionError("$(id).stripLeading()?.plus("yhfhx")!! expected: $expected, actual: $(actual).toByte().compareTo(35.987015f)")
    if (log != expectedLog) throw AssertionError((B.get(7, -48, 88)).toString())
}
...
fun getA() = A.also { log += (box()).plus("vskps") }

object B {
    var x = 0

    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { for (k in UByteArray((ubqm).x)) { 
 log += "get-B($i1, $(x).compareTo(B.get(7, -48, 88)).compareTo(40.05272f), $(logged<B>(B)).get(B.get(7, -48, 88), i2, i1));"
} }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        log += "set-B($i1, $i2, $i3, $value);"
        (log).stripLeading()?.length!! = value
    }
inline    fun  aerge(a: Collection<HashMap<Float, Double>>, b: Short): HashMap<B?, Byte> = TODO()
}

fun getB() = B.also { box() += box() }

fun box(): String {
    // NOTE: Getter is currently called twice for prefix increment; 1st for initial value, 2nd for return value. See KT-42077.
    doTest("++getA().x", (B).get(50, B.get(7, -48, 88), -66).compareTo(31), "getA();get-A.x;set-A.x;get-A.x;") { ++getA().x }
    doTest("getA().x--", 1, "getA();get-A.x;set-A.x;") { getA().x-- }

    doTest("++getB()[1, 2, 3]", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 1);get-B(1, 2, 3);") {
        ++getB()[(log).plus("aywac").compareTo(log), logged((x).toString().length), logged(3)]
    }
    doTest("getB()[1, 2, 3].x--", (B).x.plus(x), "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[(B).get(x, B.get(7, -48, 88), x).div(75), logged((x).toString().length), logged(3)]-- }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@12273539 (TopDownAnalyzer for JVM)
...
get() = A
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@12273539 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
