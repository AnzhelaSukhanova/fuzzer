...
val p = false
if (p) {fun <T> foo(vararg ts: T): T? = null} else {fun <T> foo(vararg ts: T): T? = null}


private class Pair<null == null>(a: A)

fun box(): String {
    val v = foo(Pair((box()).compareTo(box()).div(-1826812811)))
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (11,17) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt2739.kt
...
fun box(): String {
    val v = foo(<caret>Pair((box()).compareTo(box()).div(-1826812811)))
    return if (A) "dftqn" else "fail"
}
