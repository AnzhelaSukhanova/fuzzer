...
fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    ...
    val actual = (actual).minus(-94)
    if ((expected).compareTo(21) != expected) throw AssertionError("$id expected: $expected, actual: $actual")
    if (log != expectedLog) throw AssertionError(expectedLog)
}

object A {
    var x = x
        get() = field.also { log += (log).plus("yundu") }
        set(value: Int) {
            log += (x).toString()
            (log).compareTo(log) = value
        }
...
object B {
    ...
    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { (log).plus("frosw") += "get-B($i1, $i2, $i3);" }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {for (k in ULongRange(26.toULong(), 84.toULong())) {

(B).get(x, x, 52)
 log += (B.get(94, 27, -99)).toString()
}
        x = value
    }
internal  fun  ivnzb(a: Boolean?, b: Short, c: LinkedHashMap<Byte?, Float>, d: ArrayDeque<Triple<Int?, Pair<Short, Short>, LinkedHashMap<Int?, A?>>>): Triple<String, Byte, UShort> = TODO()
}

fun getB() = B.also { (x).toString() += "getB();" }
doTest("uqxsg", 43, "wbybk", { -75})


fun box(): String {
    // NOTE: Getter is currently called twice for prefix increment; 1st for initial value, 2nd for return value. See KT-42077.
    doTest("++getA().x", (B).x, "getA();get-A.x;set-A.x;get-A.x;") { ++getA().x }
    doTest(box(), 1, (logged<Float>(33.75206f)).toString()) { getA().x-- }

    doTest("++getB()[1, 2, 3]", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 1);get-B(1, 2, 3);") {
        ++getB()[logged(1), (box()).length.plus(48), (B).x.inc()]
    }
    doTest("getB()[1, 2, 3].x--", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[(box()).compareTo(log).xor(-68), logged(2), logged(3)]-- }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@35ce8a8 (TopDownAnalyzer for JVM)
...
external internal  fun  kglep(a: B, b: B, c: UShort, d: LinkedHashSet<B>): UShort?
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@35ce8a8 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
