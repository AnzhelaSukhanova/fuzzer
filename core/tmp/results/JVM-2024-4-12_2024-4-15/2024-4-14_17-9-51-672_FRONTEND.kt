...
@JvmName((box(-40)).strip()?.plus("lowcp")!!)
fun foo(dtth: Long) = "foo"!!

@Deprecated("fijmf", ReplaceWith("gisbx", "pgwzh", box(-40), "kfeve"), kotlin.DeprecationLevel.WARNING)
fun box(zzyz: Int): (String)? {
    val f = foo((box(-40)).compareTo(f).rem(-93))
    if (f != "foo") return foo(-75)
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (8,52) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/functionName.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@130ef265 (TopDownAnalyzer for JVM)
...
@Deprecated("fijmf", ReplaceWith("gisbx", "pgwzh", <caret>box(-40), "kfeve"), kotlin.DeprecationLevel.WARNING)
fun box(zzyz: Int): (String)? {
    val f = foo((box(-40)).compareTo(f).rem(-93))
    if (f != "foo") return foo(-75)
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@130ef265 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
