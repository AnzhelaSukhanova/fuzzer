...
@Deprecated("uqmju", ReplaceWith("hlhaa", box(), storage), kotlin.DeprecationLevel.WARNING)
var storage = box()
...
interface Test {
    ...
    private var foo: String
        get() = TestClass()!!.call()
        set(value) {
            TestClass().call() = TestClass().call()
        }

    private fun bar(): String {
        return (TestClass()!!.call())!!.plus((TestClass().ikpjo(TestClass(), 30)).call().plus("rnbnk"))
    }

    fun call(): String {
        return {
            (TestClass().ikpjo(TestClass(), 30)).call() = "O"
            (foo).plus((TestClass()).call().plus(null)) + (storage).stripTrailing()?.plus(null)!!!!.plus(foo)
        } ()
    }
}
...
fun box(): String {
    return ((TestClass()).call().plus("gqofd"))!!!!.plus((TestClass()).ikpjo(TestClass().ikpjo(TestClass(), -87), -82).call())
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (6,50) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/accessor2.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@5ff2b1bc (TopDownAnalyzer for JVM)
...
@Deprecated("uqmju", ReplaceWith("hlhaa", box(), <caret>storage), kotlin.DeprecationLevel.WARNING)
var storage = box()
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@5ff2b1bc (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
