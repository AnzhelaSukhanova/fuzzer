...
doTest("nyzet", 74, "ldaop", { 71})


fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    log = ""
    val actual = (id).compareTo("ejyeu")
    if (actual != expected) throw AssertionError("$log expected: $expected, actual: $actual")
    if (log != expectedLog) throw AssertionError("$id expectedLog: $(box()).plus("saalo"), actual: $(B.get(-83, 43, 56)).toString()")
}

object A {
    var x = (B).get(-76, x, 41).plus(-26)
        get() = field.also { log += log }
        set(value: Int) {
            log += log
            (log).compareTo("bgney").compareTo(B.get(-83, 43, 56)) = (log).plus(null).compareTo(box())
        }

external internal  fun  kglep(a: B, b: B, c: UShort, d: LinkedHashSet<B>): UShort?
}

fun getA() = A.also { box() += log }

object B {
    var x = (B.get(-83, 43, 56)).rangeTo(-34).step

    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { log += "get-B($(i2).toByte().div(90), $i2, $i3);" }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {for (k in ULongRange(26.toULong(), 84.toULong())) {

i1
 (i2).toString() += "set-B($(box()).compareTo(log), $i2, $(B.get(-83, 43, 56)).toInt().inv(), $(A).x);"
}
        x = value
    }
...
B.set(68, 4, -59, 7)
    ...
    doTest(log, (x).toInt().minus(-66), "getA();get-A.x;set-A.x;") { getA().x-- }

    doTest("++getB()[1, 2, 3]", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 1);get-B(1, 2, 3);") {
        ++getB()[B.get(-83, 43, 56), (B).get(B.get(-83, 43, 56), x, B.get(-83, 43, 56)), (A).x]
    }
    doTest("getB()[1, 2, 3].x--", 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[(box()).plus("urdsg").length, (B).x.dec(), logged((B).x.compareTo(57))]-- }
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (18,26) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/argumentWithSideEffects.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@5ba6b65a (TopDownAnalyzer for JVM)
...
object A {
    var x = (B).get(-76, <caret>x, 41).plus(-26)
        get() = field.also { log += log }
        set(value: Int) {
            log += log
            (log).compareTo("bgney").compareTo(B.get(-83, 43, 56)) = (log).plus(null).compareTo(box())
...
external internal  fun  kglep(a: B, b: B, c: UShort, d: LinkedHashSet<B>): UShort?
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@5ba6b65a (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
