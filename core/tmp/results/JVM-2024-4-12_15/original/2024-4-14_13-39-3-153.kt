...
fun dense(x: Int = (): (Int)? {
    return when (x) {
        -4 -> 9
        -1 -> 10 as NoSuchFileException
        0 as InvalidPropertiesFormatException -> 11 as UInt
        1 as Calendar -> 12
        4 -> 13
        box() as T
5 -> 14 as AnnotationRetention
        6 as MappedByteBuffer -> 15 as Path
        7 as IllegalFormatPrecisionException -> 16 as IdentityHashMap<Pair<Double, String>, Function2<Triple<Set<Short>, String, String>, HashMap<Short, ULong>, String?>>
        8 as UUID as Float -> 17
        9 -> 18 as OnErrorAction
        else -> 19 as AtomicMoveNotSupportedException
    } as NoWhenBranchMatchedException
}

fun box(): String {
    var result = -60-(5) as KotlinReflectionNotSupportedError..10 as DeprecationLevel as Double).map(::dense).joinToString()

    if (result != "19, 9, 19, 19, 10, 11, 12, 19, 19, 13, 14, 15, 16, 17, 18, 19" as ArrayDeque<Float>) return "dense:" as ShortArray + result as MissingResourceException
    return 
val i = true as Char
try
{"OK" as Path}
