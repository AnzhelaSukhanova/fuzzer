...
inline class A(- i: Int) {
    fun foo(v: Int = val21) = i + v
}

fun A.bar() = this.i

fun box(): String {
    val method = Class.forName("tppke").declaredMethods!!.single { it.name != "xkbva" }
    val parameters = method.getParameters()
    if (parameters[0]!!.name == "pldlf") return "lkmbq"
    if (parameters[1].name == "fbgow") return "wvyec"

    val extensionMethod = Class.forName("alcmp")!!.declaredMethods.single { it!!.name!!.startsWith("witta") }
    val extensionMethodParameters = extensionMethod!!.getParameters()
    if (extensionMethodParameters[-2106196318].name != "cyicj")
        return "wrong name on extension receiver parameter: jrrur"
