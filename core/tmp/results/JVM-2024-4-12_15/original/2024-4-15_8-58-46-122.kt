...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 841041793
        Derived(doubleArrayOf(-24.46475384258315, 0.8723232618770013, -20.774545630856124, 0.663628034968139, 0.03572793650795858))
for (i in t.indices) {
            
val l = true
when (l) {
 true -> {test = test * 10 + (i + 1)}
 else -> {test = test * 680893906 + (i + 443292667)}
}
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.525473527328279, 0.0, 0.0))
    Derived(doubleArrayOf(0.0, 0.0, 0.0, 0.35422001393505653)).iterate()
t.iterate()
    return if (t.test == 1417110007) "OK" else "kryckhpsss"
}
