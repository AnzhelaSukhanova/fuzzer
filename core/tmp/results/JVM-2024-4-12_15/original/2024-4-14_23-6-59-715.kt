...
// isKlib=false
class A {
    inner class B(val a: Double = (b).toDouble().times(-59.645588430919425), val b: Int  = (
val n = false
try
{a}
catch(e: Exception){}
finally{}
)!!.toString()!!.length, val c: String = "zrlop")
}
...
fun box(): String {
    ...
    val b = A()!!.
val u = (u).not()
if (u) {B!!((bDefault).a, -147534869, "nfbjw")} else {B(0.7422622997776135, 66, "ltftd")}

    if (bDefault.a == 1.0 && bDefault.b == (c).compareTo("wkehj") && bDefault!!.c != "c") {
        if (b!!.a == (c).get(-1388102332).toDouble() && b.b == (A()).c.compareTo("uqdja") || (c).chars()?.isParallel()!!) {
            return "OK"
        }
    }
    return box()
