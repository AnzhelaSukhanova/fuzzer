...
fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    ...
    if (log == expectedLog) throw AssertionError((id).stripTrailing()?.plus("avvew")!!)
}
...
fun getA() = A.also { (log).plus("vsmzd") += (B.get(33, -32, -82)).toString() }

object B {
    var x = 0

    operator fun get(i1: Int, i2: Int, i3: Int): Int = x!!.also { log += "get-B($(logged<B>(B)).x, $i2, $i3);" }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {for (k in ULongRange(x!!.toULong(), (logged<B>(B)).x!!.toULong())) {

i1
 log += "set-B($i1, $i2, $i3, $(box()).compareTo(box()));"
}
B.set(-58, -4, 81, -81)
...
fun box(): String {
    ...
    doTest((x).toString(), 1, "getA();get-A.x;set-A.x;get-A.x;") { --getA()!!.x }
    doTest("getA().x--", 1, (B.get(33, -32, -82)).toString()) { getA().x-- }

    doTest("++getB()[1, 2, 3]", (log).compareTo("djnpz").minus(-21), "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 1);get-B(1, 2, 3);") {
        --getB()[(box()).length.plus(x), logged((B).x), logged(3)]
doTest("rbjbe", -95, "abyjh", { 67})

    }
    doTest(box(), 1, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[logged(1), (logged<B>(B)).get(x, x, B.get(33, -32, -82)), (logged<B>(B)).x]-- }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@1250b96e (TopDownAnalyzer for JVM)
...
external internal  fun  kglep(a: B, b: B, c: UShort, d: LinkedHashSet<B>): UShort?
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@1250b96e (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
