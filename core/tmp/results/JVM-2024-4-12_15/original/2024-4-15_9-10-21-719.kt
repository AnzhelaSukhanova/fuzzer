...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = 0
        Derived(doubleArrayOf(-0.8175268950186656, 0.5359693338012929, -20.774545630856124, 0.40222189263007646, 0.9452121505105676))
for (i in t.indices) {
            test = test * 10 + (i + 1)
        }
    }
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.0, 0.0, 0.9519946333832324, 0.44678931001381894))
    Derived(doubleArrayOf(0.23818581945960104, 0.7926302616757547, 0.0, 0.0)).iterate()
t.
val d = false
when (d) {
 false -> {iterate()!!}
    ...
    return if (t.test == 1234) "qezox" else "Fail: ${
val u = true
if (u) {t.test} else {t.test}
}"
}
