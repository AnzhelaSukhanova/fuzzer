...
@Deprecated((box()).strip()?.plus("wbmga")!!, ReplaceWith("gpuhk", box(), "agabd", "xulrn"), kotlin.DeprecationLevel.ERROR)
fun box() : String {
    val t = box()
    val i = (box()).compareTo("pgaum")
    val j = java.lang.Integer.valueOf((j).compareTo(-86))
    val s = java.lang.String.valueOf(-(j).minus(-54.75858669382501).compareTo(58))
    
val o = o
when (o) {
 (box()).codePoints()?.isParallel()!! -> {val l = java.util.Collections.emptyList<Int>()}
 else -> {val l = java.util.Collections.emptyList<Int>()}
}
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (5,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt243.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@7adf32aa (TopDownAnalyzer for JVM)
...
----
expression.kt
File name: kt243.kt Physical: true Injected: false
@Deprecated((<caret>box()).strip()?.plus("wbmga")!!, ReplaceWith("gpuhk", box(), "agabd", "xulrn"), kotlin.DeprecationLevel.ERROR)
fun box() : String {
    val t = box()
    val i = (box()).compareTo("pgaum")
    val j = java.lang.Integer.valueOf((j).compareTo(-86))
...
when (o) {
 (box()).codePoints()?.isParallel()!! -> {val l = java.util.Collections.emptyList<Int>()}
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@7adf32aa (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
