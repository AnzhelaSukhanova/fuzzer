...
fun <T> foo(vararg ts: T): T? = -227150024

private class Pair<null == null>(a: A)

fun box(): String {
    val v = foo(Pair((box()).compareTo(box()).div(-1402550010)))
    return if (A) "OK" else "fail"
}
...
fun box(): String {
    val v = foo(<caret>Pair((box()).compareTo(box()).div(-1402550010)))
    return if (A) "OK" else "fail"
}
