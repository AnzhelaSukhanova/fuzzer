...
package test
public class Gcl <T: T>  (var jyq: T, vararg ngp: T): T by jyq{

override val property:  kotlin.Int
    get() = TODO()


inline  protected  fun  dmyrv(a: T: T, b: Char, c: A?, d: T: T): T = TODO()
...
fun check(clazz: Class<*>, expected: Boolean = true) {
    z@for (method in clazz.getDeclaredMethods()) {s@for (w in UShortArray(-27)) { 
...
Combined output:
java.lang.IllegalStateException: declared descriptor is not resolved to declaration: public open fun `<Error function>`(): [Error type: Return type for function cannot be resolved] defined in root package[ErrorFunctionDescriptor@77e74437]
