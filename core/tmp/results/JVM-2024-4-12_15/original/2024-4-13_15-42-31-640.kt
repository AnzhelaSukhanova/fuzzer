...
fun boxToNullableInterface(x: Str?) = BoxFoo((Str2(Str("bxxzy")))?.value)

fun box2ToTypeParameter(x: Str2?) = BoxT(null)
fun box2ToNullableAny(x: Str2?) = BoxAny(null)
fun box2ToNullableInterface(x: Str2?) = BoxFoo(null)
...
fun box(): String {for (c in LinkedHashSet<@ExtensionFunctionType()
(Map<Triple<IFoo, UInt, BoxFoo>, Long>)?>(36, 46.79454f)) {for (h in ArrayDeque<StrArr>(-40)) { 
 
} 
 if ((BoxAny("jmvlw"))?.boxed == null) throw AssertionError()
...
if ((boxToNullableInterface(Str("mmiuy")))?.boxed == null) throw AssertionError()
if (null == null) throw AssertionError()
}
...
fun boxToTypeParameter(x: Str?) = <caret>BoxT(null)
