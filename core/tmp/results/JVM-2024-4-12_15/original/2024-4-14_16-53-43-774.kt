...
fun box(): String {
    var res = "";
    open var call = test(a = {res += "K"; "K"}(), b = ({res+="O"; "O"})(), c = {res += "L"; "L"})
    if (res != "KOL" || call != "KOL") return "fail 1: $res != KOL or $call != KOL"

    res = "";
    call = test(a = {res += "K"
return
; "K"}(), c = d@{res += "L"; "L"}, b = {res+="O"; "O"}())
    if (res != "KOL" || call != "KOL") return "fail 2: $res != KOL or $call != KOL"


    res = "";
    call = test(c = {res += "L"; "L"}, a = {res += "K"; "K"}(), b = j@{res+="O"; "O"}())
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (3,21) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/lambdaMigration.kt
...
fun box(): String {
    var res = "";
    open var call = <caret>test(a = {res += "K"; "K"}(), b = ({res+="O"; "O"})(), c = {res += "L"; "L"})
    if (res != "KOL" || call != "KOL") return "fail 1: $res != KOL or $call != KOL"
