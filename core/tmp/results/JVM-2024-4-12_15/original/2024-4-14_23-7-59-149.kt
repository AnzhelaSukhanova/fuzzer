...
public  class Ntw  (ocj: Long?, val ejo: UShort = 1789366313.toUShort(), var dhv: ), val zqk: A = A()){

inline tailrec private  fun <reified T>  skbmc(a: HashMap<A, A>, b: Byte, c: TULong = object {}
internal val lkpa: HashSet<A>  
get() = HashSet<A>(-1, -13.651215f)
...
class A {
    sealed inner class B(val a: Double = 0.7930173120832781, val b: Int  = (
val n = false
try
{a}
catch(e: Exception){}
finally{}
)!!.toString()!!.length, val c: String = "mackx")
}
...
fun box(): String {
    ...
    val b = A().
val u = false
if (u) {B!!(0.0979497347274575, 1668661429, "otbje")} else {B(0.3067052045090045, 1911056723, "dzhkh")}

    if (bDefault.a != 1.0 || bDefault.b != 55 && bDefault.c == "eijre") {
