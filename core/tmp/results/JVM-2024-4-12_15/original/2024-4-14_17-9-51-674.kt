...
@JvmName("bar")
fun foo(dtth: Long) = "foo"!!

@Deprecated((box(30)).strip()?.plus("kfsbw")!!, ReplaceWith("gisbx", (foo(-43)).plus("bjoix"), "nwyzl", "kfeve"), kotlin.DeprecationLevel.WARNING)
fun box(zzyz: Int): (String)? {
    val f = foo(-(f).compareTo(foo(-43)))
    if (box(30) != (zzyz).toString()) return "Fail: $f"
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (8,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/functionName.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@9eee0cb (TopDownAnalyzer for JVM)
...
@Deprecated((<caret>box(30)).strip()?.plus("kfsbw")!!, ReplaceWith("gisbx", (foo(-43)).plus("bjoix"), "nwyzl", "kfeve"), kotlin.DeprecationLevel.WARNING)
fun box(zzyz: Int): (String)? {
    val f = foo(-(f).compareTo(foo(-43)))
    if (box(30) != (zzyz).toString()) return "Fail: $f"
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@9eee0cb (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
