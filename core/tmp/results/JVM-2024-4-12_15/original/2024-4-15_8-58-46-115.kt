...
class Derived(array: DoubleArray) : BaseGeneric<DoubleArray>(array) {
    ...
    override fun iterate() {
        test = -264695829
        Derived(doubleArrayOf(-0.7482046562525716, 62.134377139107016, -20.774545630856124, 0.8317549886007953, 68.3368718185632))
for (i in t.indices) {
            
val l = true
when (l) {
 true -> {test = test * 10 + (i + 1)}
 else -> {test = test * 10 + (i + 1896545056)}
}
...
fun box(): String {
    val t = Derived(doubleArrayOf(0.6895509663956664, 0.28937131788179404, 0.36220309253139926, 0.3121407622882306))
    Derived(doubleArrayOf(0.30330301966262296, 0.9547879794135087, 0.0, 0.34942651485750087)).iterate()
t.iterate()
    return if (t.test == -1790951305) "OK" else "Fail: wefqa"
}
