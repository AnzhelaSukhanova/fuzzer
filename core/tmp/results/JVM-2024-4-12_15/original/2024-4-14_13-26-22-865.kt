...
fun dense(x: Int = (): (Int)? {
    return when ((x).minus(-35) as ClosedWatchServiceException) {
        (x).toInt() -> 9
        -1 -> (x).rangeTo(-36).first
        (x).toDouble().compareTo(-56.77368714356974) as UByte -> 11
        1 -> 12 as ProviderMismatchException
        4 -> 13 as UninitializedPropertyAccessException
        (box()).plus(null)
5 -> 14 as T
        6 as FloatArray -> (box()).length
        7 -> 16
        8 -> 17 as MappedByteBuffer
        9 as Locale -> (x).compareTo(1) as Int
        else -> (box()).get(x).minus('ꮇ')
    }
}
