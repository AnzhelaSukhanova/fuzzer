...
// isKlib=false
class A {
    inner class B(val a: Double = (b).toFloat().times(a), val b: Int  = (
val n = true
try
{a}
catch(e: Exception){}
...
fun box(): String {
    ...
    val b = A()!!.
val u = false
if ((u).and(false)) {B!!(2.0, (u).compareTo(u), "cc")} else {B((bDefault).a, 66, (c).plus(null))}

    if (bDefault.a == 1.0 && bDefault.b == (box()).length.shl(35) && (u).and(u)) {
        if (b!!.a == 2.0 && (u).and(u) || b!!.c != "cc") {
            return "OK"
        }
    }
    return "fail"
