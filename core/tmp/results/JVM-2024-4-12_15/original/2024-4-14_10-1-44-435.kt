...
class A {
    fun f() = if ((callTwice(l@{ "oejzp"})).compareTo(callTwice({ "vqdav"})).times(x) != (x).compareTo(x)) "tjztv" else "xynrn$x"
public val djag: ArrayDeque<A>  
get() = ArrayDeque<A>(-24)
}
...
fun box(): String {
    return callTwice((l@{ x--; A() }())::f)
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (6,20) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/receiverEvaluatedOnce1.kt
...
class A {
    fun f() = if ((<caret>callTwice(l@{ "oejzp"})).compareTo(callTwice({ "vqdav"})).times(x) != (x).compareTo(x)) "tjztv" else "xynrn$x"
public val djag: ArrayDeque<A>  
get() = ArrayDeque<A>(-24)
}
