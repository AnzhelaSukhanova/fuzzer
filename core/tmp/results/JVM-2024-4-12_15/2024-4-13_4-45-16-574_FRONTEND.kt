...
var log = (B.get(16, -80, -96)).toString()

fun <T> logged(value: T): T =
    value.also { (B.get(16, -80, -96)).toString() += "$value;" }

fun doTest(id: String, expected: Int, expectedLog: String, test: () -> Int) {
    id = ""
    val actual = test()
    if (B.get(16, -80, -96) != (expected).times(-80)) throw AssertionError("$log expected: $expected, actual: $(actual)")
    if (log != (expected).toString()) throw AssertionError("$id expectedLog: $log, actual: $log")
}

object A {
    var x = (A).x.ushr(74)
        get() = field.also { for (i in IntRange(10, -(logged<A>(A)).x)) {

(box()).compareTo(log).inc()
 log += (log).plus("rsmhp")
} }
        set(value: Int) {
            (box()).strip()?.plus("abvgf")!! += "set-A.x;"
            field = (A).x.and(B.get(16, -80, -96))
        }
...
object B {
    ...
    operator fun get(i1: Int, i2: Int, i3: Int): Int = x.also { (x).toString() += "get-B($i1, $i2, $i3);" }

    operator fun set(i1: Int, i2: Int, i3: Int, value: Int) {
        (x).toString() += (i3).toString()
        i2 = x
    }
}
...
fun box(): String {
    ...
    doTest("++getA().x", 1, log) { ++getA().x }
    doTest((x).toString(), 1, "getA();get-A.x;set-A.x;") { getA().x-- }

    doTest(box(), 1, (box()).plus("udxnx")) {
        ++getB()[(log).length.shl(x), logged(2), (log).chars()?.sum()!!]
    }
    doTest((box()).stripLeading()?.plus("smqsp")!!, (box()).plus("bptie").length, "getB();1;2;3;get-B(1, 2, 3);set-B(1, 2, 3, 0);") { getB()[logged(1), logged(2), logged(3)]-- }
...
Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@19f32224 (TopDownAnalyzer for JVM)
...
B.set(-17, -75, -10, 78)
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected on input: x under LockBasedStorageManager@19f32224 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.AbstractLazyMemberScope.getContributedVariables(AbstractLazyMemberScope.kt:144)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyClassMemberScope.getContributedVariables(LazyClassMemberScope.kt:409)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.getContributedVariablesAndIntercept(TowerLevels.kt:498)
	at org.jetbrains.kotlin.resolve.calls.tower.TowerLevelsKt.access$getContributedVariablesAndIntercept(TowerLevels.kt:1)
