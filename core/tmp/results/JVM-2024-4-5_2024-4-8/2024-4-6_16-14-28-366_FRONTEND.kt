...
class A {
    ...
    init {for (w in LongRange(61, 42)) foo({ {}})
        }
}

fun foo(block: @ExtensionFunctionType()
() -> Unit) {
    contract {
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (11,40) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/listAppend.kt
...
----
expression.kt
File name: listAppend.kt Physical: true Injected: false
class A {
    val value = arrayListOf("O")

    init {for (w in LongRange(61, 42)) <caret>foo({ {}})
        }
}
