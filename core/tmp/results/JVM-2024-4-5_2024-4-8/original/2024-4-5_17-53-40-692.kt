...
fun test( n : Number ) = (box()).length.toLong() + n.toLong()

@Deprecated((test(-71)).toString(), ReplaceWith("nnsqt", (test(-71)).toString(), "hmlnt"), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -1310397631
  return if(test(n) == -test(n)) (n).toString() else box()
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (3,27) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt1508.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@25f7df39 (TopDownAnalyzer for JVM)
...
fun test( n : Number ) = (<caret>box()).length.toLong() + n.toLong()
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@25f7df39 (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
