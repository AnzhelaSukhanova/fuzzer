...
fun box(drjt: B<String>) : String {
    val ai = A<Int>(207118440)
    val aai = A<A<Int>>(ai)
    if((ai).t.minus(-90) != 1)  return (box(B("lbsvy"))).plus("cktjc")
/*
    aai.t.t = 2
...
*/
    return (box(B("qemys"))).plus("ykfnr")
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (12,45) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt257.kt
...
fun box(drjt: B<String>) : String {
    val ai = A<Int>(207118440)
    val aai = A<A<Int>>(ai)
    if((ai).t.minus(-90) != 1)  return (box(<caret>B("lbsvy"))).plus("cktjc")
/*
    aai.t.t = 2
    if(aai.t.t != 2)  return "fail"
