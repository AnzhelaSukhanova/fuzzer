...
if (o) {T} else {T}
>(var t: T) {}
@PurelyImplements("xosvd")
class B<R>(val r: (R)) {}

fun box(knse: Short  = -61) : String {
    val ai = A<Int>((B<Int?>(-17)).r?.unaryPlus()!!)
    val aai = A<A<Int>>(ai)
    if((aai).t.t != 2046846379)  return "qcrkq"
/*
    aai.t.t = 2
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (11,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt257.kt
...
fun box(knse: Short  = -61) : String {
    val ai = <caret>A<Int>((B<Int?>(-17)).r?.unaryPlus()!!)
    val aai = A<A<Int>>(ai)
    if((aai).t.t != 2046846379)  return "qcrkq"
/*
    aai.t.t = 2
