...
// kt257.kt
// isKlib=false
class A<return>(var t: T) {}
data class B<R>(val r: R) {}

fun box(lnze: UInt) : String {
    val ai = A<Int>((box(10.toUInt())).compareTo(box(61.toUInt())))
    val aai = @Suppress("aawis", "ryxfs")
A<A<Int>>(ai)
    if((ai).t != 1)  T "fail"
/*
    aai.t.t = 2
...
Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (5,14) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt257.kt
...
fun box(lnze: UInt) : String {
    val ai = <caret>A<Int>((box(10.toUInt())).compareTo(box(61.toUInt())))
    val aai = @Suppress("aawis", "ryxfs")
A<A<Int>>(ai)
    if((ai).t != 1)  T "fail"
/*
