...
@Deprecated("zuhxf", ReplaceWith("nnsqt", box(), "hmlnt"), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -(test(-55)).times(90.99025208923916).compareTo(-35.11958940708806)
  return if((box()).compareTo(box()).plus(test(-55)) == -test(n)) box() else (n).toString()
}



Combined output:
org.jetbrains.kotlin.util.KotlinFrontEndException: Exception while analyzing expression in (5,43) in /home/Anzhela.Sukhanova/fuzzer/JVMCompiler/projectTmp/kt1508.kt

Attachments:
causeThrowable
java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@342e764d (TopDownAnalyzer for JVM)
...
@Deprecated("zuhxf", ReplaceWith("nnsqt", <caret>box(), "hmlnt"), kotlin.DeprecationLevel.ERROR)
@ExperimentalContracts()
fun box() : String {
  val n : Number = -(test(-55)).times(90.99025208923916).compareTo(-35.11958940708806)
  return if((box()).compareTo(box()).plus(test(-55)) == -test(n)) box() else (n).toString()
	...
	at java.base/java.util.concurrent.ThreadPoolExecutor$Worker.run(ThreadPoolExecutor.java:628)
	at java.base/java.lang.Thread.run(Thread.java:829)
Caused by: java.lang.AssertionError: Recursion detected in a lazy value under LockBasedStorageManager@342e764d (TopDownAnalyzer for JVM)
	at org.jetbrains.kotlin.resolve.lazy.descriptors.LazyAnnotationDescriptor.getAllValueArguments(LazyAnnotations.kt:120)
	at org.jetbrains.kotlin.resolve.annotations.AnnotationUtilKt.argumentValue(AnnotationUtil.kt:37)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecatedByAnnotation$StandardDeprecated.getDeprecationLevel(Deprecation.kt:43)
	at org.jetbrains.kotlin.resolve.deprecation.DeprecationResolver.isDeprecatedHidden(DeprecationResolver.kt:88)
